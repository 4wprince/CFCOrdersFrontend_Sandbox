 ...
 * v5.9.2 - Header layout: Order# → Date → Red Pill → Days, AI summary single-spaced
 */
...
const OrderCard = ({ order, onOpenDetail, onOpenShippingManager, onUpdate }) => {
  ...
  const handleStatusChange = async (e) => {
    ...
    try {
      ...
      if (!updates) throw new Error('Unknown status')
  ...
  return (
    <div className="order-card" style={{ borderLeftColor: status.color }}>
      <div className="order-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 5px', gap: '12px' }}>
        <div className="order-id" onClick={() => onOpenDetail && onOpenDetail(order)} style={{ fontWeight: '600', cursor: 'pointer', marginLeft: '5px' }}>
          #{order.order_id}
        </div>
        {orderDate && <div className="order-date" style={{ color: '#666', fontSize: '13px' }}>{orderDate}</div>}
        <select className="status-dropdown" value={order.current_status || 'needs_payment_link'} onChange={handleStatusChange} onClick={(e) => e.stopPropagation()} disabled={isUpdating} style={{ backgroundColor: status.color + '20', color: status.color, borderColor: status.color, padding: '4px 8px', borderRadius: '12px', border: '1px solid', fontWeight: '600', fontSize: '12px', cursor: 'pointer' }}>
          {STATUS_OPTIONS.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
        </select>
        <span className="order-age-pill" style={{ backgroundColor: '#333', color: 'white', padding: '4px 10px', borderRadius: '12px', fontSize: '12px', fontWeight: '600', marginRight: '5px' }}>{getAgeLabel(order.order_date)}</span>
      </div>
        ...
        <div className="order-financials">
          ...
          {shippingTotals.customerCharge > 0 && (
            <div className="shipping-summary">
              ...
              {shippingTotals.profit !== 0 && <span className={'ship-profit ' + (shippingTotals.profit >= 0 ? 'positive' : 'negative')}>({shippingTotals.profit >= 0 ? '+' : ''}${shippingTotals.profit.toFixed(2)})</span>}
            </div>
          )}
          {totalDisplay && shippingTotals.customerCharge > 0 && <div className="grand-total">Total: ${(orderTotal + shippingTotals.customerCharge).toFixed(2)}</div>}
        </div>
        {warehouses.length > 0 && <div className="warehouses">{warehouses.map((wh, i) => <span key={i} className="warehouse-tag">{wh}</span>)}</div>}
      </div>

      {order.shipments && order.shipments.length > 0 && (
        <div className="order-shipments">
          {(order.shipments || []).map((shipment, i) => <ShipmentRow key={shipment.shipment_id || i} shipment={shipment} order={order} onOpenShippingManager={onOpenShippingManager} onUpdate={onUpdate} />)}
        </div>
      )}

      <div className="order-notes-section">
        {order.comments && <div style={{ ...containerBase, backgroundColor: '#fffde7', border: '1px solid #f9a825' }}><strong>Customer:</strong> {order.comments}</div>}
        <div style={{ ...containerBase, backgroundColor: '#f3e5f5', border: '1px solid #9c27b0' }}>
          {isEditingNotes ? (
            <div>
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Add internal notes..." rows={3} onClick={(e) => e.stopPropagation()} style={{ width: '100%', marginBottom: '8px', padding: '6px', borderRadius: '4px', border: '1px solid #9c27b0', fontSize: '13px', fontFamily: 'inherit', resize: 'vertical' }} />
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={(e) => { e.stopPropagation(); handleSaveNotes(); }} disabled={isSavingNotes} style={{ backgroundColor: '#4caf50', color: 'white', border: 'none', padding: '4px 12px', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' }}>{isSavingNotes ? 'Saving...' : 'Save'}</button>
                <button onClick={(e) => { e.stopPropagation(); setIsEditingNotes(false); setNotes(order.notes || ''); }} disabled={isSavingNotes} style={{ backgroundColor: '#9e9e9e', color: 'white', border: 'none', padding: '4px 12px', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' }}>Cancel</button>
              </div>
            </div>
          ) : (
        ...
        {order.ai_summary && <div style={{ ...containerBase, backgroundColor: '#e8f5e9', border: '1px solid #4caf50' }}><strong>AI Summary:</strong><div style={{ marginTop: '4px', whiteSpace: 'pre-wrap', wordWrap: 'break-word', overflowWrap: 'break-word', lineHeight: '1.3' }}>{formatAISummary(order.ai_summary)}</div></div>}
      </div>
    </div>
  )
}
